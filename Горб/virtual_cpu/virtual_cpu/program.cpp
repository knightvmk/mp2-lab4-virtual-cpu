...
{
	if (_count > max_t_count) throw ("Fail. PC will be overdrived");
	count = _count;
	complex = 1;
	task = new double[count];
	srand(time(0));
...
{
	active = true;
	double speed = SeeFreq()*0.0017;
	int size_clust = SeeSize();
	int percpu = SeeCorePerCPU();
	tact = err = completes = 0;
	bool gotcha = false;
	TQueue<double> work(30);
	srand(time(0)); //toggle randomizer
	while (active)
	{
		for (register int i = 0; i < count*complex; i++, tact++)  //main cycle 
		{
			if (i < count)
			{
